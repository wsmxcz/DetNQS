...
def create_logpsi_evals(
    ...
    *,
    ...
    Spin-flip symmetry reduces NN evaluations by ~50% using Z2 orbit representatives.
    Normalization: log(ψ) → log(ψ/‖ψ‖) where ‖ψ‖² = Σ|ψᵢ|²
    ...
    Args:
        ...
        spin_flip_symmetry: Enable Z2 spin-flip reduction
        feat_s: S-space features (required if symmetry is True)
        feat_c: C-space features (required if symmetry is True)
    ...
    def _batch_apply(params: PyTree, features: jnp.ndarray) -> jnp.ndarray:
        """Vectorized model application with checkpointing."""
    ...
    def _norm_s(log_s: jnp.ndarray) -> jnp.ndarray:
        """Normalize S-space wavefunction: log(ψ) → log(ψ/‖ψ‖)."""
        if not normalize:
            return log_s
    ...
    def _norm_full(log_all: jnp.ndarray, n_s_count: int) -> jnp.ndarray:
        """Normalize full wavefunction across S and C spaces."""
        if not normalize:
            return log_all
    ...
    if use_sym:
        if feat_s is None or feat_c is None:
            raise ValueError("feat_s and feat_c must be provided when spin_flip_symmetry=True")
    ...
    if use_sym:
        ...
        def eval_s(params: PyTree, _: jnp.ndarray) -> jnp.ndarray:
        ...
        def eval_full(params: PyTree, _s: jnp.ndarray, _c: jnp.ndarray) -> jnp.ndarray:
            """Evaluate full log(ψ) using symmetry reduction."""
            log_rep = _chunked_apply(params, feat_rep)
            log_rep = jnp.asarray(log_rep, dtype=device_complex)
            log_s = log_rep[idx_map_s]
        ...
        def eval_full(params: PyTree, feat_s: jnp.ndarray, feat_c: jnp.ndarray) -> jnp.ndarray:
            """Evaluate full log(ψ) with runtime features."""
...
def _prepare_spinflip_maps(
    ...
    """
    Construct unique Z2 orbit representatives for spin-flip symmetry.
    
    Maps (α,β) and (β,α) to same representative using canonical ordering.
    Returns: (feat_rep_jax, index_map_s_jax, index_map_c_jax)
    """
    ...
    # Canonicalize: (min(a,b), max(a,b)) maps (a,b) and (b,a) to same key
    ...
    _, idx_unique, indices = np.unique(keys, return_index=True, return_inverse=True)
